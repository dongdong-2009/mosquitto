...
/**********************************************************************
 * 函数功能	：解析broker推送过来的命令，执行设备操作，返回操作结果
 * 参数		：订阅主题之后，收到broker推送的解析之后的命令
 * 作者		：mkk
 * 修改时间	：2016-12-02
 **********************************************************************/
int publish(char *str)
{
	...
	/************************************************************
	 * 应该在此制定XGO自己的数据协议，对不同的命令执行不同的操作
	 ************************************************************/
	if(!strcmp(str,"on"))
	{
		str = "The led have on!!!";
	...
	//发布执行结果过程中，应该对于不同的错误形成不同的错误码并返回结果
...
/*******************************************************************************
 * 函数功能	：订阅主题，注册给订阅端的回调函数，将订阅收到的消息回传
 * 参数		：订阅线程回传的broker推送消息
 * 作者		：mkk
 * 修改时间	：2016-12-02
 *******************************************************************************/
int publish_callback(char *rcv_data)
{
	...
	/******************************************************************
	 * 1、此处应该对broker推送的消息解析成设备识别的命令字符串
	 * 2、创建线程将命令传给命令解析接口
	 ******************************************************************/
	...
	//给线程收尸，发布结束，broker会断开tcp连接和mqtt连接
...
/***************************************************************
 * 函数功能	：主题订阅，用于设备和应用服务器数据交互
 * 作者		：mkk
 * 修改时间	：2016-12-02
 * 备注		：订阅的主题暂时由柏峰返回给设备，然后初始化全局的\
 			  订阅主题和发布主题
 ***************************************************************/
int subscribe(void)
{
