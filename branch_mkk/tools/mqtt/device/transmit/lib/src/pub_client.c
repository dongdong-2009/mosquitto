...
#include <mosquitto_sub_pub.h>		//add by mkk

#include <pthread.h>				//test pthread
...
static char *pub_parm[] = {"pub","-h","","-t","","-m",""};
...
int mosquitto_pub_init(Pub_Data *pub_s)
{
	...
	if(NULL == pub_s->topic)
		...
		pub_parm[4] = pub_s->topic;

	if(NULL == pub_s->msg)
		return 1;
	else
		pub_parm[6] = pub_s->msg;

	ret = client_config_load(&cfg,CLIENT_PUB,sizeof(pub_parm)/sizeof(char *),pub_parm);
	if(ret)
	{
...
//int main(int argc, char *argv[])
{
	...
	char buf[1024];
	//mosquito代表一个连接的客户端，所有的客户端都保存到服务器的业务子树中
	struct mosquitto *mosq = NULL;
	int rc;
	int rc2;

	//第二个参数代表发布消息，该api和sub客户端公用
...
#if 0
void* deal_pub_1(void *parm)
{
	char *argv[] = {"pub","-t","event/ideahos/server_1/device_1","-m","aaaaa"};
	client_pub(5,argv);
	pthread_exit(NULL);
}

void* deal_pub_2(void *parm)
{
	char *argv[] = {"pub","-t","event/ideahos/server_1/device_1","-m","bbbbb"};
	client_pub(5,argv);
	pthread_exit(NULL);
}

//线程去调用client_pub不会出现发布多次之后，订阅者收不到发布的消息
int main(void)
{
	pthread_t pid;
	int ret;
	...
	while(1)
	{
		ret = pthread_create(&pid,NULL,deal_pub_1,NULL);
		if(ret)
		{
		...
		ret = pthread_create(&pid,NULL,deal_pub_2,NULL);
		if(ret)
		{
			printf("pthread_create failed\n");
			return -1;
...
#if 0
//直接调用函数会出现，调用多次之后，订阅者收不到发布者的消息
int main(int argc,char **argv)
{
	//char *argv[] = {"pub","-t","event/ideahos/server_1/device_1","-m","------"};
	//char *argv1[] = {"pub","-t","event/ideahos/server_1/device_1","-m","mmmmm"};
	//char *argv2[] = {"pub","-t","event/ideahos/server_1/device_1","-m","aaaaa"};
	//char *argv3[] = {"pub","-t","event/ideahos/server_1/device_1","-m","bbbbb"};


	while(1)
	{
		argv[argc-1] = "aaaaaaaaaaaaaaaa";
		client_pub(argc,argv);
		sleep(1);
		argv[argc-1] = "bbbbbbbbbbbbbbbb";
		client_pub(argc,argv);
		sleep(1);
	}
...
#if 0
//直接拿shell脚本去调用可以实现多次发布之后订阅者仍然能收到发布者发布的消息
int main(int argc,char **argv)
{
	client_pub(argc,argv);
	return 0;
