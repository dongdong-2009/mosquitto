...
int main(int argc, char *argv[])
{
	...
	//=============================mqtt3_db_open()生成订阅数总根和两个子树====================================/
	//1、业务子树和总根的topic字段为空，一个_mosquitto_subhier结构代表一个节点
	//2、系统子树的topic字段为$SYS
	//3、系统topic在系统发布消息时创建，业务topic在订阅或发布时创建
	//========================================================================================================/
	...
	//====================在此生成系统子树的两个分支$SYS/broker/version和$SYS/broker/timestamp====================/
	//									  系统子树生成过程调用api
	//												  |
	...
	//============================================================================================================/
#ifdef WITH_SYS_TREE
	if(config.sys_interval > 0){ //他的值在前面设置为10
	...
	listener_max = -1;
	listensock_index = 0;
	//printf("listen_count:%d\n",config.listener_count);	//在解析函数中++为1
	...
	signal(SIGQUIT, handle_sigint);		//add by mkk
#ifdef SIGHUP
	...
	run = 1;
	rc = mosquitto_main_loop(&int_db, listensock, listensock_count, listener_max);	//后两个参数为2,4
