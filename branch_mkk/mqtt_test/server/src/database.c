...
#if 0
	/********************************************************************************
	 * 发送的消息会经过此处，可以在此设置过滤信息
	 * 将context->msgs成员赋值NULL并循环清空该消息，并缓存给LWAN
	 * 		_message_remove(db, context, &tail, last);
	 ********************************************************************************/
	if(context->msgs != NULL && context->msgs->store != NULL && context->msgs->store->topic != NULL && context->msgs->store->payload != NULL)
	{
		printf("主题:%s,消息：%s[%d]\n",context->msgs->store->topic,(char *)context->msgs->store->payload,context->msgs->store->payloadlen);
		//对收到的消息（如语音）调用第三方接口，转换成应用服务器可以识别的信息
		//暂时全部缓存给主进程注册的回调函数
		//strncpy(context->msgs->store->payload,"xxxxxxxxxxxxxxxxxxxxxxxxxxxxx",context->msgs->store->payloadlen);
	}
#endif
