...
#include <data_type.h>		//add by mkk 2016-12-1
...
int mosquitto_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max,Data_filter *mqtt_data_filter)
{
	//printf("sock:%d,count:%d\n",*listensock,listensock_count);
#ifdef WITH_SYS_TREE
		...
		HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){
			...
			if(context->sock != INVALID_SOCKET){
				...
				if(!(context->keepalive)
					...
					//将消息挂载到客户端context对应的消息队列中
					if(mqtt3_db_message_write(db, context,mqtt_data_filter) == MOSQ_ERR_SUCCESS){
