...
int mqtt3_db_message_write(struct mosquitto_db *db, struct mosquitto *context,Data_filter *mqtt_data_filter)
{
	int rc;
	struct mosquitto_client_msg *tail, *last = NULL;
	uint16_t mid;
	 ...
	 * broker发布数据过滤
	 *****************************************************************************/
	if(context->msgs && context->msgs->store && context->msgs->store->topic && context->msgs->store->payload)	
	{
		if(!strncmp(context->msgs->store->topic,"common",strlen("common")))
		{
			mqtt_data_filter->common_data_filter(context->msgs->store->payload,context->msgs->store->payloadlen);
		}
		else if(!strncmp(context->msgs->store->topic,"audio",strlen("audio")))
		{
			mqtt_data_filter->audio_data_filter(context->msgs->store->payload,context->msgs->store->payloadlen);
		}
		else if(!strncmp(context->msgs->store->topic,"media",strlen("media")))
		{
			mqtt_data_filter->media_data_filter(context->msgs->store->payload,context->msgs->store->payloadlen);
		}
	}
