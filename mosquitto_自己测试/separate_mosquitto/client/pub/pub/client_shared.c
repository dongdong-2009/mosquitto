...
int client_id_generate(struct mosq_config *cfg, const char *id_base)
{
	...
	if(cfg->id_prefix){
		...
		//printf("cfg->id:%s\n",cfg->id); //cfg->id = "mosquitto/3237-fyql";
	}
	return MOSQ_ERR_SUCCESS;
}
...
int client_connect(struct mosquitto *mosq, struct mosq_config *cfg)
{
	...
	if(cfg->use_srv){
		...
		//rc:0,host:192.168.100.122,port:1883,bind_address:(null)
		//printf("rc:%d,host:%s,port:%d,bind_address:%s\n",rc,cfg->host,cfg->port,cfg->bind_address);
	}
#else
	rc = mosquitto_connect_bind(mosq, cfg->host, cfg->port, cfg->keepalive, cfg->bind_address);
#endif
