...
#if 0	
	//模拟设备登陆服务器，带着token，验证token通过才能继续mqtt通讯
	char tmp[36] = {0};
	write(sock,"token",strlen("token"));
	read(sock,tmp,36);
	printf("broker_message:%s\n",tmp);
#endif
...
ssize_t _mosquitto_net_write(struct mosquitto *mosq, void *buf, size_t count)
{
	...
	//printf("buf:%s\n",strlen(buf));
	//写了一个字节
	//write(mosq->sock, buf, count);	//会导致返回值小于0(-1),对方已经关闭socket，触发了SIGPIPE信号，会使进程结束
	//write(mosq->sock, buf, count);	
...
#  endif
		...
		if(((packet->command)&0xF6) == PUBLISH){
			...
			//pthread_mutex_unlock(&mosq->current_out_packet_mutex);		//add by mkk
			//return MOSQ_ERR_SUCCESS;										//add by mkk
...
#else
	//this way go
	//sv数组每个元素代表一个socket端口
	int sv[2];

	//创建一对匿名socket用于本地进程间通信
