...
#ifndef WIN32
	//当publish报文发送两次时，第一次发送完broker会关闭socket，第二次write会返回-1,并触发SIGPIPE信号，关闭当前进程，下面的设置是忽略这种信号
...
static int _mosquitto_connect_init(struct mosquitto *mosq, const char *host, int port, int keepalive, const char *bind_address)
{
	...
	//printf("mosq->sockpairR:%d,mosq->sockpairW:%d\n",mosq->sockpairR,mosq->sockpairW);
	return MOSQ_ERR_SUCCESS;
}
...
int mosquitto_connect_bind(struct mosquitto *mosq, const char *host, int port, int keepalive, const char *bind_address)
{
	...
	//return 2;		//专门返回非0测试连接broker失败会发生什么
	//Unable to connect (A network protocol error occurred when communicating with the broker.).
	if(rc) return rc;
...
int mosquitto_loop(struct mosquitto *mosq, int timeout, int max_packets)
{
	...
	if(mosq->sock != INVALID_SOCKET){
		...
		maxfd = mosq->sock;
		//接受数据，可能是broker的mqtt connect返回报文
		//在client_connect api中建立mosq->sock连接并发送mqtt connect报文
	...
	if(fdcount == -1){
		...
		if(mosq->sock != INVALID_SOCKET){
			if(FD_ISSET(mosq->sock, &readfds)){
				...
				{
					do{
						//进来,可能是检测到broker的返回mqtt connack报文
...
#endif
				{
					//写完mosq->sock则关闭socket,mosq->sock = INVALID_SOCKET
#if 1
