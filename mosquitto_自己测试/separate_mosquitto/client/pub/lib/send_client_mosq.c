...
int _mosquitto_send_connect(struct mosquitto *mosq, uint16_t keepalive, bool clean_session)
{
	...
	payloadlen = 2+strlen(clientid); //=19
	//printf("payloadlen:%d\n",payloadlen);
	if(mosq->will){
		will = 1;
	...
	packet->remaining_length = headerlen+payloadlen;  //31
	//printf("head:%d\npayload:%d\nlen:%d\n",headerlen,payloadlen,packet->remaining_length);
	//生成固定头部：connect报文（byte1），剩余长度（byte2）
