...
int mqtt3_socket_accept(struct mosquitto_db *db, mosq_sock_t listensock)
{
...
#if 0
	//测试代码
	char str[24] = {0};			//测试客户端与broker建立socket连接但是没有建立MQTT连接之前的通讯
	if(read(new_sock,str,24) < 0)
		printf("error\n");
	...
	if(!strcmp(str,"token"))
		...
		printf("this is a unvalid custom\n");
#endif
...
int mqtt3_socket_listen(struct _mqtt3_listener *listener)
{
	...
	hints.ai_flags = AI_PASSIVE;	//该socket用于bind
	...
	//printf("listener_sock_count-------->%d\n",listener->sock_count);	//2
